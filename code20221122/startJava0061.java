...
public class startJava0061 {
    ...
    //--rps : 가위, 바위, 보, 이긴 횟수...
    //2) 컴퓨터 시점 흐름(플로우차트)
    //3) 플로우차트에 맞춰 클래스 구성 (instance, static, extends...)

    //startjava0061 : 메인메서드 
    //startJava0061_account : 회원1명의 정보
    //startJava0061_account_extends_log_ver : account상속받아 log정보 입력 클래스
    //-- 필드 : id/pw/email/name
    ...
    static ArrayList<startJava0061_account_extends_log_ver> accounts = new ArrayList<>();
    ...
    private static void changePw() {
        ...
        //checkList : 중복값이 있으면 true반환
        //accounts배열 안에 몇 번째 있는지 확인
    ...
    private static int checkList_array(String input_id) {
        ...
        for(int i =0; i<accounts.size(); i++) {
            //찾고자 하는 id값이 같다고 하면 몇 번째 배열에 있는지 (i) return
            if(accounts.get(i).id.equals(input_id)) {
                return i;
            }
        }
        //같은 값이 없으면 -1
    ...
    private static void signUp() {
            infoMenu();
            String input_id = inputScanner();
            //상속받은 자식 클래스 타입으로 인스턴스 생성
            startJava0061_account_extends_log_ver ac1 = new startJava0061_account_extends_log_ver(
    ...
    private static void loginGame() {
        ...
        if(result.equals("FALSE")) {
            ...
            loginUser = inputId;
            loginUserIndex = checkList_array(inputId);
            accounts.get(loginUserIndex).getInfo();
            gameStart();
        }
    }
