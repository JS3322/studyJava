...
public class startJava0062_liteVersion {
    ...
    private static void startGameModelVSCom() {
        ...
        //모델링 적용 게임에만 실행
        if (winPlayer1) {
            startJava0062_liteVersion_log.winModelPlayer ++;
        } else {
            startJava0062_liteVersion_log.loseModelPlayer ++;
        }
    }
    ...
    private static int random_model0001() {
        ...
        //전체 승리 횟수
        int totalCount = startJava0062_liteVersion_log.WinPaperCount +
                startJava0062_liteVersion_log.WinScissorsCount +
                startJava0062_liteVersion_log.WinRockCount;
...
//        int WinRock = startJava0052_liteVersion_log.WinRockCount / totalCount;
        ...
        System.out.println("Paper count = "+ startJava0062_liteVersion_log.WinPaperCount);
        System.out.println("Scissors count = "+ startJava0062_liteVersion_log.WinScissorsCount);
        System.out.println("Rock count = "+ startJava0062_liteVersion_log.WinRockCount);
        System.out.println("모델 측정 랜덤 수치 : "+result);
        //랜덤으로 값을 출력하고 그 값이 가위,바위,보 범위 안에 들어가면 0,1,2 값을 리턴하여 최적의 가위바위보를 출력
        if(result > 0 & result <= startJava0062_liteVersion_log.WinScissorsCount) {
            return 0;
        } else if (result > startJava0062_liteVersion_log.WinScissorsCount &
                result <= startJava0062_liteVersion_log.WinScissorsCount + startJava0062_liteVersion_log.WinRockCount) {
            return 1;
        }else if (result > startJava0062_liteVersion_log.WinScissorsCount + startJava0062_liteVersion_log.WinRockCount) {
            return 2;
        }
        return -1;
    }
    ...
    private static void getLog() {
        startJava0062_liteVersion_log.setLog();
        startJava0062_liteVersion_log.getLog();
    }

    private static void getModel0001Log() {
        startJava0062_liteVersion_log.setModel0001Log();;
        startJava0062_liteVersion_log.getsetModel0001Log();;
    }
